...
#Import HDF5 file
rawdata = pd.read_hdf('...\EurexOptionsDax.h5')
...
data['ImpliedVola'] = -1
def BSMOptionSigma(sigma, Price, PutCallFlag, S, T, K, r):
    return Price - BSMOption(PutCallFlag, S, T, K, r, sigma, Greek='Price')

for i in range(len(data)):
    data['ImpliedVola'].iloc[i] = optimize.bisect(BSMOptionSigma, a=-0.01, b=1, args=(data['TaeglicherAbrechnungspreis'].iloc[i], data['OptionType'].iloc[i], data['SchlusspreisBasiswert'].iloc[i], data['t_Delta'].iloc[i], data['StrikePrice'].iloc[i], data['EONIA'].iloc[i]), xtol=1e-8, maxiter=500)
