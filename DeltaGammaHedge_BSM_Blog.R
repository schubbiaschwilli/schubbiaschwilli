setwd("D:/h_da/_Blog/Themen/Options/BSM_Delta_Gamma_Hedging")
...
# Define Scatterplot PayOff vs Hedge
   png(filename="ScatterplotErrors.png", width=900, height=600, pointsize=12)
	layout(c(1))
	...
	# Legende formatieren
	legend(
		if(PutCallFlag=="Call"){min(S_t)} else max(S_t)*3/4, max(Payoff),
		legend = c("Payoff","DeltaHedge","DeltaGammaHedge"), col = c("green", "red","blue"), pch=c(19,19,19), cex=1)
	dev.off()
	
# Define Histogramms Errors and Boxplot Errors
	png(filename="BoxPlotErrors.png", width=1200, height=800, pointsize=40)
...
# Test auf Normalverteilung
	# http://de.wikibooks.org/wiki/GNU_R:_shapiro.test
	# ls(Error_DeltaHedge_Shapiro)

	# Error_DeltaHedge_Shapiro
	Error_DeltaHedge_Shapiro <- shapiro.test(Error_DeltaHedge)

	if(Error_DeltaHedge_Shapiro["p.value"] > 0.05)
	{
		print("Der p-Wert ist größer als 0.05 => somit wird die Nullhypothese angenommen, dass die Daten Normalverteilt sind")
	} else {
		print("Der p-Wert ist kleiner als 0.05 => somit liegt keine Normalverteilung vor")
	}
	
	
	# Error_DeltaGammaHedge_Shapiro
	Error_DeltaGammaHedge_Shapiro <- shapiro.test(Error_DeltaGammaHedge)

	if(Error_DeltaGammaHedge_Shapiro["p.value"] > 0.05)
	{
		print("Der p-Wert ist größer als 0.05 => somit wird die Nullhypothese angenommen, daraus resultiert, dass die Daten Normalverteilt sind")
	} else {
		print("Der p-Wert ist kleiner als 0.05 => somit liegt keine Normalverteilung vor")
	}
...
# Correlation
		cor(Error_DeltaHedge,Error_DeltaGammaHedge)
		cor.test(Error_DeltaHedge,Error_DeltaGammaHedge)

# Define QQ-Plots
   png(filename="QQPlots.png", width=600, height=400, pointsize=10)
		layout(rbind(c(1,2)))
	...
	dev.off()
		...
		#qqplot(hedgeerror, rt(300, df = 5))
		#qqnorm(precip, ylab = "hedgeerror")
